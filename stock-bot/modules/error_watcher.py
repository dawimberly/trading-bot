...
def classify_error_class(
    ...
    *,
    ...
    """Bucket: transient_api | transient_network | auth | order_reject | other."""
    if error is not None and not isinstance(error, (str, bytes)):
        try:
            ...
            status = alpaca_http_status(error)
            if status in (429, 500, 502, 503, 504) or (
                is_transient_alpaca_error(error) and not isinstance(error, AlpacaAuthError)
            ):
                if status in (429, 500, 502, 503, 504) or "internal server error" in str(
                    error
                ).lower():
    ...
    *,
    ...
    if error_class in ("transient_network", "transient_api") and now - _last_tg_at < cooldown:
        ...
        if _last_tg_fingerprint.startswith("network:") or _last_tg_fingerprint.startswith(
            ...
            return
    mode = "PAPER" if config.PAPER_TRADING else "LIVE"
    if error_class == "transient_api":
        text = (
            f"[PythonTrading {mode}] Alpaca 5xx (transient)\n"
            f"{short[:350]}\n"
            f"Broker outage — skipped this cycle; bot keeps running.\n"
            f"At: {file_line}"
        )
...
def log_exception(
    ...
    *,
    ...
    if error_class == "transient_api":
        return log_transient_api(str(exc), context=context or "exception", extra=extra)
...
def log_transient_api(
    error: str,
    *,
    context: str = "cycle",
    extra: dict[str, Any] | None = None,
) -> str:
    """Rate-limited Alpaca 5xx log — not a failed order, soft Telegram."""
    import time
    ...
    skip_queue = now - _last_network_log_at < _NETWORK_LOG_COOLDOWN_SEC
    file_line = _frame_file_line()
    problem = f"TRANSIENT_API ({context}): {error}"
    row = {
        "ts": _now_iso(),
        "id": event_id,
        "event": "transient_api",
        "mode": "paper" if config.PAPER_TRADING else "live",
        "context": context,
        "error_type": "AlpacaCriticalError",
        "error_class": "transient_api",
        "error": str(error)[:1000],
        "file_line": file_line,
        "fix_area": "modules/alpaca_client.py (broker 5xx — not strategy)",
        "stack": "(transient Alpaca 5xx — no strategy fix)",
        **(extra or {}),
    }
    if not skip_queue:
        _last_network_log_at = now
        _append_jsonl(_ERRORS_PATH, row)
        _append_jsonl(_ACTIONS_PATH, {**row, "event": "api5xx_skip"})
        try:
            _LOG_DIR.mkdir(parents=True, exist_ok=True)
            block = (
                f"\n## Alpaca 5xx {event_id} — {_now_iso()}\n\n"
                f"**Problem:** {problem[:500]}\n\n"
                f"**error_class:** `transient_api`\n\n"
                f"**Note:** Broker Internal Server Error — not a strategy error. "
                f"Bot skipped this cycle and continues.\n\n"
                f"**Location:** `{file_line}`\n"
            )
            with _CURSOR_QUEUE.open("a", encoding="utf-8") as fh:
                fh.write(block)
...
def log_failed_order(
    *,
    ...
    # Cycle-level failures used to be logged as symbol=CYCLE / side=n/a, which
    # Telegram rendered as "Order failed: n/a CYCLE". Never treat those as orders.
    fake_cycle = str(symbol).upper() == "CYCLE" and str(side).lower() in (
        "n/a",
        "na",
        "",
        "none",
    )
    if fake_cycle or str(reason) == "cycle_error":
