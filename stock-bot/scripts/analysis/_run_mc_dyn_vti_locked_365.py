"""Longer-window MC: paper-aggressive + Dynamic VTI 40-75%, 365d, 50 runs, diagnostics."""
...
def _configure_mc_isolation() -> Path:
    mc = ROOT / "data" / "cache" / "mc_v154_dyn_vti_locked_365"
...
def main() -> int:
    ...
    out = ROOT / "scripts" / "analysis" / "monte_carlo_v154_dyn_vti_locked_365.json"
    status = ROOT / "scripts" / "analysis" / "monte_carlo_v154_dyn_vti_locked_365.status.txt"
    ...
    print(
        "=== Locked Dynamic VTI MC (left-tail investigation) ===\n"
        "paper-aggressive | Dynamic VTI LOCKED 40-75% | 365d | 50 runs | "
        "no-thinking | seed=42 | VTI/regime diagnostics ON\n"
        f"export -> {out}",
        flush=True,
    )
