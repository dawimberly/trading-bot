"""Quiet wrapper: Monte Carlo on locked Realistic Research v1.5.4 (paper-aggressive)."""
...
def _configure_mc_isolation() -> Path:
    """Route runtime JSON side-effects away from live paper-bot data/."""
    mc = ROOT / "data" / "cache" / "mc_v154_locked_365"
...
def main() -> int:
    ...
    out = ROOT / "scripts" / "analysis" / "monte_carlo_v154_locked_365.json"
    status = ROOT / "scripts" / "analysis" / "monte_carlo_v154_locked_365.status.txt"
    ...
    print(
        "=== Locked Realistic Research v1.5.4 MC ===\n"
        "paper-aggressive | 365d | 100 runs | no-thinking | seed=42\n"
        f"export -> {out}",
        flush=True,
    )
