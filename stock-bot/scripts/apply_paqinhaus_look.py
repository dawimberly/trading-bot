"""Apply Paqinhaus poster look to stock-bot dashboards. Look only. Idempotent."""
...
def main() -> int:
    if APP.is_file():
        ...
        if after != before:
            ...
            print(f"look: {APP.name} already Paqinhaus", flush=True)
    if STREAMLIT.is_file():
        ...
        if after != before:
            ...
            print(f"look: {STREAMLIT.name} already Paqinhaus", flush=True)
    return 0
