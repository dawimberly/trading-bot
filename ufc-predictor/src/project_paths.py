...
def load_env_files(root: Path) -> None:
    ...
    # Frozen EXE in dist/: also load project-root .env (dist/../.env)
    if getattr(sys, "frozen", False):
        candidates.insert(1, root.parent / ".env")
...
def bootstrap(*, entry_file: Path | None = None) -> Path:
    """
    ...
    config.ODDS_API_KEY = os.getenv("THE_ODDS_API_KEY") or os.getenv("ODDS_API_KEY", "")
    return root
